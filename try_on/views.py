...
pdf_file_path = os.path.join('media', 'user_pdfs', 'user_srush4_wardrobe_info.pdf')
...
def chatbot(request):
     ...
     return render(request, 'try_on/chatbot.html', context)
...
def login_view(request):
    ...
    return render(request, 'try_on/login.html', {'form': form})
...
def register_view(request):
    ...
    return render(request, 'try_on/signup.html', {'form': form})
...
def profile(request):
    ...
    return render(request, 'try_on/profile.html', {'user_info': profile})
...
def user_wardrobe(request):
    ...
    # Get all clothes
    unworn_items = WardrobeItem.objects.filter(last_worn__lt=six_months_ago)
...
def sell(request):
    ...
    return render(request, 'try_on/sell.html', context)
